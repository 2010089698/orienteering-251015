...
describe('ClassOrderPanel world ranking integration', () => {
  it('loads a world ranking CSV and toggles target classes', async () => {
    renderWithStartlist(<ClassOrderPanel />, {
      initialState: { entries, settings: baseSettings, startlistId: 'SL-1' },
    });

    const fileInput = screen.getByLabelText('世界ランキングファイル (CSV)');
    ...
    expect(
      statusMessages.some((node) => node.textContent?.includes('世界ランキングを 2 件読み込みました。')),
    ).toBe(true);

    const m21Checkbox = await screen.findByLabelText('M21');
    expect(m21Checkbox).not.toBeDisabled();
    expect(m21Checkbox).not.toBeChecked();

    await userEvent.click(m21Checkbox);

    expect(
      await screen.findByText('世界ランキング対象クラス: M21'),
    ).toBeInTheDocument();
  });
