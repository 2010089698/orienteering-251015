...
} from '../../../startlist/StartlistDtos.js';
...
const baseSettingsDto = toStartlistSettingsDto(baseSettings);
const baseLaneAssignmentsDto = baseLaneAssignments.map(toLaneAssignmentDto);
const baseClassAssignmentsDto = baseClassAssignments.map(toClassAssignmentDto);
const baseStartTimesDto = baseStartTimes.map(toStartTimeDto);

test('StartlistSettingsEnteredEvent copies the provided data', () => {
  const occurrence = new Date(baseDate);
  const settings = JSON.parse(JSON.stringify(baseSettingsDto)) as typeof baseSettingsDto;
  const event = new StartlistSettingsEnteredEvent('startlist-immutable', settings, occurrence);

  occurrence.setUTCFullYear(2030);
  settings.intervals.laneClass.milliseconds = 999;
  settings.intervals.classPlayer.milliseconds = 999;
  settings.laneCount = 99;

  assert.strictEqual(event.occurredAt.toISOString(), baseDate.toISOString());
  assert.deepStrictEqual(event.settings, baseSettingsDto);
});

test('LaneOrderAndIntervalsAssignedEvent copies input arrays and date', () => {
  const occurrence = new Date(baseDate);
  const laneAssignments = [...baseLaneAssignmentsDto];
  ...
  assert.strictEqual(event.laneAssignments.length, baseLaneAssignmentsDto.length);
  assert.deepStrictEqual(event.laneAssignments, baseLaneAssignmentsDto);
  assert.strictEqual(event.occurredAt.toISOString(), baseDate.toISOString());
});
...
test('PlayerOrderAndIntervalsAssignedEvent copies input arrays and date', () => {
  const occurrence = new Date(baseDate);
  const classAssignments = [...baseClassAssignmentsDto];
  const event = new PlayerOrderAndIntervalsAssignedEvent(
    'startlist-immutable',
    classAssignments,
    occurrence,
  );
  ...
  assert.strictEqual(event.classAssignments.length, baseClassAssignmentsDto.length);
  assert.deepStrictEqual(event.classAssignments, baseClassAssignmentsDto);
  assert.strictEqual(event.occurredAt.toISOString(), baseDate.toISOString());
});
...
test('ClassStartOrderManuallyFinalizedEvent copies input arrays and date', () => {
  const occurrence = new Date(baseDate);
  const classAssignments = [...baseClassAssignmentsDto];
  const event = new ClassStartOrderManuallyFinalizedEvent(
    'startlist-immutable',
    classAssignments,
    occurrence,
  );
...
test('StartTimesAssignedEvent copies input arrays and date', () => {
  const occurrence = new Date(baseDate);
  const startTimes = [...baseStartTimesDto];
  ...
  assert.strictEqual(event.startTimes.length, baseStartTimesDto.length);
  assert.deepStrictEqual(event.startTimes, baseStartTimesDto);
  assert.strictEqual(event.occurredAt.toISOString(), baseDate.toISOString());
});
...
test('StartlistFinalizedEvent copies the provided snapshot, arrays, and date', () => {
  ...
  snapshot.laneAssignments.push(
    toLaneAssignmentDto(
      LaneAssignment.create({ laneNumber: 3, classOrder: ['class-c'], interval, laneCount: 3 }),
    ),
  );
  snapshot.classAssignments.splice(0, snapshot.classAssignments.length);
  snapshot.startTimes.splice(0, snapshot.startTimes.length);

  assert.strictEqual(event.finalStartlist.laneAssignments.length, baseLaneAssignmentsDto.length);
  assert.deepStrictEqual(event.finalStartlist.laneAssignments, baseLaneAssignmentsDto);
  assert.strictEqual(event.finalStartlist.classAssignments.length, baseClassAssignmentsDto.length);
  assert.deepStrictEqual(event.finalStartlist.classAssignments, baseClassAssignmentsDto);
  assert.strictEqual(event.finalStartlist.startTimes.length, baseStartTimesDto.length);
  assert.deepStrictEqual(event.finalStartlist.startTimes, baseStartTimesDto);
  assert.strictEqual(event.occurredAt.toISOString(), baseDate.toISOString());
});
